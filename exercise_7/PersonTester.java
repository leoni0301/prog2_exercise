...
public class PersonTester {
	...
	public static void main(String[] args) {
		...
		TreeMap <MyDate, HashSet<Person>> orderedPpl1 = new TreeMap<MyDate, HashSet<Person>>();
		...
		for (Person p: persons) {
			...
			if(orderedPpl1.containsKey(p.getBirthday())){
				...
				HashSet<Person> newPpl = new HashSet<Person>();
				newPpl.add(p);
		...
		TreeMap <MyDate, TreeSet<Person>> orderedPpl2 = new TreeMap<MyDate, TreeSet<Person>>(new MonthDayComparator());
		for (Person p: persons) {
			MyDate curBirth = p.getBirthday();
			...
			if(orderedPpl2.containsKey(p.getBirthday())){
				...
				TreeSet<Person> newPpl = new TreeSet<Person>();
				newPpl.add(p);
